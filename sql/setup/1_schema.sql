...
CREATE TABLE `login_bonus_masters` (
    ...
    `column_count` int(
        2
    ) NOT NULL COMMENT '何日分用意するかの日数。例:7日のスタートダッシュ、20日の通常ログイン',
...
CREATE INDEX user_id_present_idx ON user_present_all_received_history (
    user_id, present_all_id
);
...
CREATE TABLE `user_items` (
    ...
    `item_type` int(
        1
    ) NOT NULL COMMENT 'アイテム種別:1はusersテーブル、2はuser_cardsへ。3,4をこのテーブルへ保存',
    `item_id` int NOT NULL COMMENT 'アイテムID',
    `amount` int NOT NULL COMMENT 'アイテム数',
    `created_at` bigint NOT NULL,
    `updated_at` bigint NOT NULL,
...
CREATE TABLE `item_masters` (
    ...
    `item_type` int(
        2
    ) NOT NULL COMMENT '1:ISUCOIN、2:ハンマー（カード)、3:強化素材、4:時短アイテム（タイマー）',
